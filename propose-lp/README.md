...
- すべての写真：本物の写真素材がまだ無いため、ロケーションごとに配色とモチーフ（ヤシの木・波・ドーム・気球など）を変えたインラインSVGの旅行ポスター風イラストをプレースホルダーとして表示しています（右下に小さくロケーション名も表示）。外部画像を一切読み込まないため完全オフラインで表示され、実写に差し替える前提の設計です（`assets/js/propose-data.js` の `sceneSVG` / `silhouettePath`）。
