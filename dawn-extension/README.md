...
## ④ プラン商品もテンプレート複製せずに増やせるように（今回分）

プランの数が多いため「プランを1つ作るたびにテンプレートを複製」する
運用は避けたい、というご相談を受け、`product-plan.liquid`をメタ
フィールド駆動のデュアルモードに変更しました。都市ページ(area-lp)は
テンプレート複製方式を継続する判断でしたが、こちらは商品のメタ
フィールドという、複製しなくても商品ごとに内容を切り替えられる
Shopify標準機能がそのまま使えるため、そちらに寄せています。

- **メタフィールドを設定した商品**：ヒーロー文言・含まれるもの／
  含まれないもの・オプション・スケジュール・FAQ・注意事項・ギャラリー
  画像が、すべて商品ごとのメタフィールドから表示されます。**特徴タグは
  商品の「タグ」欄をそのまま使う**ので設定不要です
- **メタフィールドを設定していない商品**：今まで通りテンプレート内の
  セクション設定・ブロックが使われます（既存のダナン・ロンドンの
  ページはそのまま、何も壊れません）

つまり、**新しいプランは「商品を作る→同じ`location-plan`テンプレート
を選ぶ→メタフィールドに内容を入力」だけで追加でき、テンプレートを
複製する必要がなくなりました。** セットアップ手順は
`docs/product-plan-metafields-setup.md`にまとめています。
