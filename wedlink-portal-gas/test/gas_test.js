...
section('50. 希望日の時間帯(AM/PM)・複数プラン希望・お客様情報の追加項目・チェックリスト追加');
{
  ...
  ctx.apiSaveFieldsQuiet(jpToken, kanri, {
    ...
    '希望日①場所': 'トレヴィの泉', '希望日②場所': 'ヴェッキオ橋'
    // 希望日③プラン・場所は空欄のまま（従来どおりの使い方も引き続きできることの確認用）
  });
  d = ctx.apiGetReservationDetail(jpToken, kanri).detail;
  ...
  check('この時点では案件全体の撮影希望場所もまだ変わらない（希望日ごとの欄とは別管理）', !d['撮影希望場所']);
  ...
  check('確定しなかった希望日①のプラン欄はそのまま残る（上書きされない）',
        d['希望日①プラン'] === 'ローマ3時間フォト');
  // ★不具合修正の確認：これまで場所だけ自動反映されず、記入欄タブの単独欄を手動で
  // 書き直さない限り希望日ごとの場所と食い違ったままになっていた
  check('確定した希望日②の場所が、案件全体の撮影希望場所欄へ自動反映される（不具合修正）',
        d['撮影希望場所'] === 'ヴェッキオ橋', d['撮影希望場所']);
  check('確定しなかった希望日①の場所欄はそのまま残る（上書きされない）',
        d['希望日①場所'] === 'トレヴィの泉');
  ...
  ctx.apiSaveFieldsQuiet(jpToken, created2.kanriNo, { 'プラン名': '既存プラン', '撮影希望場所': '既存の場所' });
  ctx.apiSaveFieldsQuiet(vieToken, created2.kanriNo, { '希望日① STS 支店': 'OK' });
  const d2 = ctx.apiGetReservationDetail(jpToken, created2.kanriNo).detail;
  check('希望日にプラン指定が無ければ、確定してもプラン名は上書きされない（既存の値のまま）',
        d2['プラン名'] === '既存プラン', d2['プラン名']);
  check('希望日に場所指定が無ければ、確定しても撮影希望場所は上書きされない（既存の値のまま）',
        d2['撮影希望場所'] === '既存の場所', d2['撮影希望場所']);
...
section('84. 【機能追加】全拠点の書き込める履歴欄・列席・撮影データのアップ・方面・過去の退避（項目101）');
{
  ...
  check('店舗の欄に手配課は書き込めない', err !== null, String(err));
  err = null;
  try { ctx.apiAddMemo(shopToken, kanri, '共有メモ（手配課）', '横取り'); } catch (e) { err = e.message; }
  check('手配課の欄に店舗は書き込めない', err !== null, String(err));
  err = null;
  try { ctx.apiAddMemo(shopToken, kanri, 'メモ（現地用）', '横取り'); } catch (e) { err = e.message; }
  check('現地の欄に店舗は書き込めない', err !== null, String(err));
  const typesFor = (token) => (ctx.apiGetReservationDetail(token, kanri).detail.memoLog || []).map(m => m.type);
  check('店舗の画面には店舗自身の記録だけが届く',
        typesFor(shopToken).join(',') === '共有メモ（日本支店）', typesFor(shopToken).join(','));
  check('店舗の画面に現地の記録は届かない',
        typesFor(shopToken).indexOf('メモ（現地用）') === -1, typesFor(shopToken).join(','));
  check('店舗の画面に手配課の記録は届かない',
        typesFor(shopToken).indexOf('共有メモ（手配課）') === -1, typesFor(shopToken).join(','));
  check('現地支店の画面に店舗の記録は届かない',
        typesFor(vieToken).indexOf('共有メモ（日本支店）') === -1, typesFor(vieToken).join(','));
  ...
  check('書き込むと記入者が自動で残る', (ctx.apiGetReservationDetail(jpToken, kanri).detail.memoLog || [])
        .filter(m => m.type === '共有メモ（日本支店）').every(m => !!m.who));
  check('書き込むと日時が自動で残る', (ctx.apiGetReservationDetail(jpToken, kanri).detail.memoLog || [])
        .filter(m => m.type === '共有メモ（日本支店）').every(m => !!m.datetime));
