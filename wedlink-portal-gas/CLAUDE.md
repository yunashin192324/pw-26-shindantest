...
支店マスタの`表示言語`列（`en`＝英語、空欄＝日本語）でBRANCHロールの行ごとに設定する
（SETUP.md 項目85。マーレ支店がその最初の適用例）。特定の支店名をコードに埋め込む設計には
していないので、他の支店に同じ対応をしたい場合もコード変更は不要（スプレッドシート上で
その支店の行に`表示言語=en`を設定するだけ）。

- 支店宛メール通知の要否は、`表示言語`とは別の`支店メール通知`列（既定ON）で制御する。
  `getBranchEmail_(branchCode)`を必ず経由させることで、支店宛の通知経路すべて
  （`sendDirectionalMail_`・納品期限アラート・未返信督促）に一括で適用される。
  **新しく支店宛メールを送る処理を追加するときは、必ず`getBranchEmail_`経由で宛先を
  取得すること**（直接`branchMetaMap_()[code].email`を参照すると、この制御が効かない）。
- 画面表示・メール本文の英訳は、固定のUIラベルを人手で英訳してコードに持たせる方式ではなく、
  **表示直前・送信直前にLanguageAppでその場で翻訳する**方式にした（画面内の日本語文言が
  1,200箇所以上あり、人手翻訳では現実的な工数に収まらなかったため）。クライアント側の
  仕組み（MutationObserverでDOMを監視し`apiTranslateBatch`へまとめて送る）は
  JavaScript.html末尾にあり、既存の画面コードには一切手を入れていない。
  **属性（`placeholder`等）の翻訳結果を書き戻すときは、テキストノードと同様に
  「直前に自分が書いた値と同じなら再翻訳しない」チェックを必ず入れること**
  （このチェックが無いと、翻訳結果に元の日本語が残るケースで英訳を無限に継ぎ足してしまう。
  実際にこの不具合を実装中に踏んだ＝`i18nLastWrittenAttr`参照）。
- `LanguageApp`はテスト環境（`gas_harness.js`）・モックアップ（`mockup_core.js`）の両方で
  呼べないため、それぞれに簡易モックを用意している。テスト側は`'EN:' + text`（「EN:」が付くか
  どうかで翻訳経路を通ったかを検証できる）、モックアップ側は`i18n_demo_dict.js`の辞書で
  部分置換して「それらしい英語画面」を見せる。
- **翻訳で書き換えてはいけない場所がある。** `<textarea>`の中身は表示用の文字ではなく編集対象の
  値そのもので、英訳で書き換えると支店側が保存した時点で日本語のデータが失われる。
  `TEXTAREA`/`SCRIPT`/`STYLE`は`inSkippedElement_`で除外済み。**表示用の文字と、フォームの値を
  兼ねている場所を新しく作るときは、必ずこの除外リストを見直すこと**（`<input>`や`<option>`は
  値を`value`属性で持つため安全。属性のうち翻訳しているのは`placeholder`/`title`/`aria-label`だけ）。
- **翻訳は1文字列につき1回の通信になる。** 画面1枚で100〜170文字列に達するため、まとめて1回の
  `apiTranslateBatch`に渡すと初回表示が何十秒も待たされる（GASの実行時間上限6分にも近づく）。
  画面側は40件ずつに分けて並行に呼び、サーバー側にも上限200件の安全網（`I18N_BATCH_LIMIT`）を
  置いてある。ここを1回にまとめ直さないこと。
- **「支店へ通知を送るか」と「支店の連絡先は何か」を混同しないこと。** 前者は`getBranchEmail_`
  （「支店メール通知」がOFFなら空文字）、後者は`branchContactEmail_`（設定に左右されない）。
  現地スタッフ手配メールの返信先のように、通知ではない用途で前者を使うと、通知OFFの支店で
  返信先が消える（実際にこの不具合を踏んだ＝SETUP.md 項目86）。
- **支店→日本方向の翻訳（英→日）は`translateEnToJa_`。対象はメッセージ・やり取り履歴のみに
  限定している。** 備考・現地記入欄のような編集可能な入力欄は対象に含めないこと。含めると、
  担当者がその欄を編集せず保存しただけで翻訳結果（日本語）がそのまま保存され、支店が書いた
  原文（英語）が失われる（項目86のtextarea不具合と同じ構造の危険）。範囲を広げたくなったら、
  「読み取り専用の表示データか、保存に使われうる値か」を必ず先に確認すること（SETUP.md 項目87）。
