...
## 通知メールと確認ダイアログの扱い（項目89で決めた方針）

- **通知メールの送信失敗で、保存済みの操作をエラーにしない。** メール送信は1日の送信上限や
  アドレスの打ち間違いで普通に失敗する。データを書き終えたあとに例外を外へ出すと、利用者には
  「保存できなかった」と見えて同じ操作をやり直し、**同じメッセージが二重に保存される**
  （実際に再現を確認済み）。`sendDirectionalMail_`は例外を外へ出さず、失敗理由を戻り値で返す
  （中身は`sendDirectionalMailCore_`）。**新しく通知を送る処理を書くときも、必ずこの関数を
  経由させること**（直接`MailApp.sendEmail`を書くと、この保護が効かない）。
  API側は`mailWarning`として理由を返し、画面は消えない帯（`#notice-bar`）で
  「保存は完了・通知だけ失敗・やり直し不要」と知らせる。**やり直しを促す文言にしないこと**
  （やり直しこそが二重保存の原因）。
- **ブラウザ標準の確認ダイアログ（`confirm`/`alert`）は自動翻訳が届かない。** 画面の自動翻訳は
  DOMの変化を監視する仕組みなので、DOMの外に出るダイアログには効かない。英語専用支店にも
  日本語のまま出てしまうため、**`confirmI18n_`／`alertI18n_`を使うこと**（`confirm`/`alert`を
  直接書かない）。`confirmI18n_`は非同期なので`await`が必要＝呼び出し元も`async`にする。
